...
Figure~\ref{fig:pattern-tree} (see Appendix) shows a small example tree for the pattern
...
\subsection{Results}
To analyse how the genetic programming loop behaves over time, we ran
two representative long-run experiments and logged the per-generation
fitness.
The first run uses a population of 16 individuals and 100 generations
under the configuration described above.
...
We then performed a second experiment with a different target audio clip
and retuned fitness weights that put more emphasis on the FFT feature.
This run uses a population of 32 individuals and 130 generations and also
logs the standard deviation of fitness.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{../data/figures/fitness_log_2.pdf}
  \caption{Evolution of fitness for the second run (population 32,
  130 generations, updated fitness weights and a different target clip).
  The blue band shows the range between minimum and maximum fitness; the
  green band shows mean $\pm$ standard deviation.}
  \label{fig:fitness-evolution-2}
\end{figure}

Even in this configuration the population makes steady progress according
to the aggregate fitness, but the best individuals do not sound close to
the new target, indicating that our hand-crafted feature weights remain
highly sensitive to timing, timbre and other details.
In contrast, the hand-crafted pattern for this target is perceptually much
closer and should ideally attain a substantially higher fitness than we
currently observe.
...
For the second run with the new target clip, the best individual recorded
in the log appears in generation~116 with scalar fitness
$\mathrm{fitness}=0.551$ and the expression
%

\begin{quote}
  \scriptsize
  \verb|d1 $ struct "t t t f t t t f t f f f t t f t" (|
  \verb|       append (rev (euclid 7 24 (fast 3 (|
  \verb|           s "hh" # n (scale "major" "0 2 5 6 7") #|
  \verb| s "superpiano")))) (|
  \verb|           s "hh")) # n "3 7"|
  \end{quote}
...
%
which layers a dense Euclidean hi-hat pattern over a major-scale
\texttt{superpiano} line with an additional structural hi-hat layer.
Despite relatively high FFT similarity, this pattern still differs
substantially from the new target in perceived rhythm and timbre, and the
overall fitness remains well below what we would expect for a convincing
match.
...
they could provide a single, perceptually grounded similarity measure and
reduce the need for delicate hand-tuning of individual feature weights.
...
syntactic correctness and musical structure, with fitness steadily improving
under both experimental configurations.
At the same time, the approach is computationally expensive because each
...
and may not always match perceptual similarity; and while the grammar
ensures syntactic validity, it does not enforce musical form, so some
high-fitness patterns still sound less satisfying than the reference.
...
reference clip.
By combining a typed Tidal grammar, musically meaningful genetic operators
and a multi-feature audio similarity fitness, the system can discover valid
patterns that converge toward the target according to both quantitative
metrics and qualitative listening.
Despite limitations due to computational cost and hand-tuned fitness
weights, the results support the feasibility of grammar-guided evolution as
a bridge between symbolic live-coding patterns and audio-domain similarity
objectives.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{../data/figures/waveforms/waveform_handcrafted_2.pdf}%
  \caption{Hand-crafted Tidal pattern for the second experiment: waveform and spectrum
           compared to the new target clip.}
  \label{fig:waveform_handcrafted_2}
\end{figure}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{../data/figures/waveforms/waveform_gen_000_2.pdf}%
  \caption{Early best candidate for the new target (generation~0,
           $\mathrm{fitness}=0.3421$).}
  \label{fig:waveform_gen000_2}
\end{figure}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{../data/figures/waveforms/waveform_gen_016_2.pdf}%
  \caption{Representative candidate for the new target (generation~16,
           $\mathrm{fitness}=0.4082$).}
  \label{fig:waveform_gen016_2}
\end{figure}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{../data/figures/waveforms/waveform_gen_069_2.pdf}%
  \caption{Mid-run candidate for the new target (generation~69,
           $\mathrm{fitness}=0.5128$).}
  \label{fig:waveform_gen069_2}
\end{figure}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{../data/figures/waveforms/waveform_gen_116_2.pdf}%
  \caption{Best evolved candidate for the new target (generation~116,
           $\mathrm{fitness}=0.551$).}
  \label{fig:waveform_best116_2}
\end{figure}
