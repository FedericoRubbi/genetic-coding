...
representative long-run experiment and logged the fitness of each generation.
The configuration for this run uses a population of
16 individuals, 100 generations, a high mutation rate of 0.95, and elitist
preservation of the top 2 individuals per generation under the
\texttt{get\_fitness} objective.
...
Despite the aggressive mutation rate, the maximum fitness curve shows a
monotonic improvement from an initially mediocre pattern toward a solution
with fitness close to 0.7, while the mean fitness oscillates in a narrower
band around 0.55.
The large spread between minimum and maximum fitness indicates that the
population maintains diversity throughout the run, which helps avoid
premature convergence while still allowing the elites to steadily push the
frontier of best solutions.

\section{Discussion}
The experiments indicate that a grammar-based genetic programming approach
can reliably steer TidalCycles patterns toward a fixed audio target while
preserving syntactic correctness and musical structure.
...
the population to steadily improve, reaching solutions with perceptually
high similarity to the target according to our multi-feature objective.
The diversity visible in the fitness range suggests that the search does not
collapse prematurely, which is important in a space as large and structured
as the Tidal grammar.

At the same time, the results highlight several limitations.
The approach is computationally expensive, since each evaluation requires
rendering audio and extracting features; this constrains population size and
the number of runs we can explore.
Moreover, the handcrafted fitness weights inevitably bias the search toward
certain musical aspects (e.g.\ timbre and harmony) and might not capture
perceptual similarity perfectly for all targets.
Finally, while the grammar ensures syntactic validity, it does not enforce
high-level musical form, so some evolved patterns, although close in
features, may still sound less musically satisfying than the reference.
...
J.~A. Biles, ``GenJam: A genetic algorithm for generating jazz solos,'' in
  \emph{Proc. Int. Computer Music Conf.}, 1994, pp.~131--137.
...
O.~A. de~la~Puente, R.~S. Alfonso, and M.~A. Moreno, ``Automatic composition
  ...
  Quad}, vol.~32, no.~2, pp.~148--155, 2002.
...
J.~Reddin, J.~McDermott, and M.~O'Neill, ``Elevated pitch: Automated
  ...
  EvoApplications (EvoMusArt)}, 2009, pp.~579--584.
...
B.~Manaris, D.~Vaughan, C.~Wagner, J.~Romero, and R.~B. Davis, ``Evolutionary
  ...
  (EvoWorkshops/EvoMusArt)}, 2007, pp.~522--532.
...
A.~Horner, J.~Beauchamp, and L.~Haken, ``Machine tongues XVI: Genetic
  ...
  Music Journal}, vol.~17, no.~4, pp.~17--29, 1993.
...
R.~N. McArthur and C.~P. Martin, ``An application for evolutionary music
  ...
  Sound, Art and Design (EvoMUSART~2021)}, 2021, pp.~142--157.
...
S.~Hickinbotham and S.~Stepney, ``Augmenting live coding with evolved
  ...
  and Design (EvoMUSART~2016)}, 2016, pp.~31--46.
...
B.~McFee, C.~Raffel, D.~Liang, D.~P.~W. Ellis, M.~McVicar, E.~Battenberg, and
  ...
  \emph{Proc. 14th Python in Science Conf.}, 2015, pp.~18--25.
